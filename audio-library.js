...
/* AJK Cover Shelf — third book-library view (covers only). Front-end only. */
(() => {
'use strict';
const GRID_ID='ajk-cover-shelf', BTN_ID='ajk-cover-shelf-toggle'; let coverMode=false;
const label=()=>((localStorage.getItem('ajk_ui_lang')||'de').toLowerCase()==='en'?'Covers':'Cover');
const findWrap=()=>document.querySelector('.book-list-wrap');
function books(w){const seen=new Set();return Array.from(w.querySelectorAll('[id^="book-card-"]')).map(card=>{if(!card.id||seen.has(card.id))return null;seen.add(card.id);const img=card.querySelector('img');const h=card.querySelector('h3');let title=(h&&h.textContent.trim())||card.id.replace(/^book-card-/,'');try{title=decodeURIComponent(title)}catch(_){}return{id:card.id,title,src:img?img.src:''};}).filter(Boolean)}
function styles(){if(document.getElementById('ajk-cover-shelf-style'))return;const s=document.createElement('style');s.id='ajk-cover-shelf-style';s.textContent=`#${GRID_ID}{display:none;grid-template-columns:repeat(auto-fill,minmax(118px,1fr));gap:24px 18px;padding:28px 0 44px;align-items:start}#${GRID_ID}.is-open{display:grid}.ajk-cover-tile{appearance:none;border:0;background:none;padding:0;cursor:pointer;min-width:0}.ajk-cover-frame{display:block;width:100%;aspect-ratio:2/3;background:var(--bone-deep,#e9e3d7);border:1px solid var(--rule,#cfc6b5);overflow:hidden;box-shadow:0 8px 22px rgba(0,0,0,.10);transition:.18s}.ajk-cover-tile:hover .ajk-cover-frame,.ajk-cover-tile:focus-visible .ajk-cover-frame{transform:translateY(-4px);box-shadow:0 14px 30px rgba(0,0,0,.16)}.ajk-cover-tile img{width:100%;height:100%;object-fit:cover;display:block}.ajk-cover-fallback{height:100%;display:flex;align-items:center;justify-content:center;padding:12px;font-family:'Cormorant Garamond',serif;font-size:15px;line-height:1.15;color:var(--ink-2,#555);text-align:center}#${BTN_ID}{white-space:nowrap;background:none;border:1px solid var(--rule);color:var(--ink-2);font-family:'Archivo',sans-serif;font-size:10.5px;letter-spacing:.08em;text-transform:uppercase;padding:10px 14px;cursor:pointer;margin:0 0 14px}#${BTN_ID}[aria-pressed="true"]{border-color:var(--gold);color:var(--gold);background:rgba(212,175,55,.07)}@media(max-width:640px){#${GRID_ID}{grid-template-columns:repeat(3,minmax(0,1fr));gap:16px 10px;padding-top:18px}}@media(max-width:380px){#${GRID_ID}{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(min-width:900px){#${GRID_ID}{grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:30px 24px}}`;document.head.appendChild(s)}
function grid(w){let g=document.getElementById(GRID_ID);if(!g){g=document.createElement('div');g.id=GRID_ID;w.parentNode.insertBefore(g,w)}const list=books(w),sig=list.map(b=>b.id+'|'+b.src).join('\n');if(g.dataset.signature===sig)return g;g.dataset.signature=sig;g.textContent='';list.forEach(b=>{const bt=document.createElement('button');bt.type='button';bt.className='ajk-cover-tile';bt.title=b.title;bt.setAttribute('aria-label',b.title+' — Details');const fr=document.createElement('span');fr.className='ajk-cover-frame';if(b.src){const im=document.createElement('img');im.src=b.src;im.alt=b.title+' cover';im.loading='lazy';fr.appendChild(im)}else{const fb=document.createElement('span');fb.className='ajk-cover-fallback';fb.textContent=b.title;fr.appendChild(fb)}bt.appendChild(fr);bt.addEventListener('click',()=>{setMode(false);requestAnimationFrame(()=>{const t=document.getElementById(b.id);if(t)t.scrollIntoView({behavior:'smooth',block:'start'})})});g.appendChild(bt)});return g}
function setMode(on){coverMode=!!on;const w=findWrap(),g=w&&grid(w),bt=document.getElementById(BTN_ID);if(!w||!g)return;w.style.display=coverMode?'none':'';g.classList.toggle('is-open',coverMode);if(bt)bt.setAttribute('aria-pressed',coverMode?'true':'false');try{localStorage.setItem('ajk_book_cover_view',coverMode?'1':'0')}catch(_){}}
function ensure(){const w=findWrap();if(!w)return false;styles();grid(w);if(!document.getElementById(BTN_ID)){const old=document.querySelector('.book-view-toggle'),bt=document.createElement('button');bt.id=BTN_ID;bt.type='button';bt.textContent=label();bt.setAttribute('aria-pressed','false');bt.addEventListener('click',()=>setMode(!coverMode));if(old&&old.parentNode)old.parentNode.insertBefore(bt,old.nextSibling);else w.parentNode.insertBefore(bt,w);try{if(localStorage.getItem('ajk_book_cover_view')==='1')setMode(true)}catch(_){}}return true}
let scheduled=false;const obs=new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;if(ensure()&&coverMode)setMode(true)})});function start(){ensure();obs.observe(document.body,{childList:true,subtree:true})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
})();
