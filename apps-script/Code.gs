/**
 * SOURCE OF TRUTH — A. J. Khan author-site Apps Script backend.
 *
 * Canonical repository copy since 21.09.2026. Changes to the Apps Script
 * backend should be made here first, reviewed/committed, and then copied to
 * the Google Apps Script project and deployed as a NEW VERSION.
 *
 * IMPORTANT: GitHub is the versioned source of truth; the Apps Script editor
 * is the deployment target. A GitHub commit is not live until deployed.
 */
...
function syncDriveForAllBooks() {
  ...
  books.forEach(function(b) {
    ...
    // metadata.json ist die kanonische strukturierte Quelle fuer Buchdaten.
    // Neben Genre werden jetzt auch Wortzahl, Kapitelzahl, optionale
    // Print-Seitenzahl und ein Text-Fallback fuer den Klappentext uebernommen.
    // Ein separates KLAPPENTEXT_-Dokument im Sprachordner hat weiterhin
    // Vorrang; files.klappentext in der JSON ist nur ein Dateipfad und KEIN
    // Klappentext-Inhalt.
    try {
      ...
      if (metaFile) {
        ...
        if (metaText) {
          try {
            ...
            logDriveSync(logSheet, b.title, 'metadata.json ungueltig (kein gueltiges JSON): ' + parseErr.message);
          }
        }
      }
      ...
      if (meta) {
        ...
        const metaWordCount = Number(meta.word_count) || 0;
        if (metaWordCount > 0 && metaWordCount !== Number(b.wordCount || 0)) {
          b.wordCount = metaWordCount;
          changed = true;
          logDriveSync(logSheet, b.title, 'Wortzahl aus metadata.json uebernommen: ' + metaWordCount);
        }

        const metaChapterCount = Number(meta.chapter_count) || 0;
        if (metaChapterCount > 0 && metaChapterCount !== Number(b.chapterCount || 0)) {
          b.chapterCount = metaChapterCount;
          changed = true;
          logDriveSync(logSheet, b.title, 'Kapitelzahl aus metadata.json uebernommen: ' + metaChapterCount);
        }

        // Reflowable EPUBs haben keine feste Seitenzahl. Nur uebernehmen, wenn
        // der Produktions-Workflow explizit eine Print-Seitenzahl liefert.
        const metaPageCount = Number(meta.page_count) || 0;
        if (metaPageCount > 0 && metaPageCount !== Number(b.pageCount || 0)) {
          b.pageCount = metaPageCount;
          changed = true;
          logDriveSync(logSheet, b.title, 'Seitenzahl aus metadata.json uebernommen: ' + metaPageCount);
        }

        // Bevorzugte Felder fuer echten Klappentext. Falls die JSON nur eine
        // logline enthaelt, dient sie als kurzer Fallback, damit die Buchkarte
        // nicht leer bleibt. Ein KLAPPENTEXT_-Dokument bleibt hoeher priorisiert.
        const metadataBlurb = [
          meta.back_cover_text, meta.blurb, meta.klappentext,
          meta.description, meta.short_description
        ].find(function(v) { return typeof v === 'string' && v.trim(); });
        const metadataLogline = (typeof meta.logline === 'string') ? meta.logline.trim() : '';
        const fallbackHook = (metadataBlurb || metadataLogline || '').trim();
        const fallbackSource = metadataBlurb ? 'metadata' : (metadataLogline ? 'metadata-logline' : '');

        if (fallbackHook && b.hookSource !== 'file' &&
            (!b.hook || b.hookSource === 'metadata' || b.hookSource === 'metadata-logline')) {
          if (fallbackHook !== b.hook || fallbackSource !== b.hookSource) {
            b.hook = fallbackHook;
            b.hookSource = fallbackSource;
            changed = true;
            logDriveSync(
              logSheet,
              b.title,
              (metadataBlurb ? 'Klappentext' : 'Logline-Fallback') + ' aus metadata.json uebernommen.'
            );
          }
        }
      } else {
      ...
      if (kind && kind !== b.kind) {
        b.kind = kind;
        changed = true;
        logDriveSync(logSheet, b.title, 'Genre uebernommen: ' + kind);
      }
    } catch (err) {
      logDriveSync(logSheet, b.title, 'Metadata-Fehler: ' + err.message);
    }
  });
...
// ============================================================================
// Bookmark sync module — inlined so a live deployment only needs Code.gs.
// Existing Bookmarks sheet is reused; no setup is required if it already exists.
// ============================================================================
...
// ============================================================================
// Audio access module — existing Access-sheet users remain source of truth.
// Inlined here so the live Apps Script deployment only needs Code.gs updated.
// ============================================================================
