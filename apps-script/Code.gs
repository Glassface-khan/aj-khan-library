 ...
 *
 * Source of truth: GitHub /apps-script/Code.gs.
 * Live runtime: Google Apps Script project; deployment remains a separate
 * manual step until an Apps-Script deployment connector is available.
 *
 * Rule: edit this GitHub file first, verify it, then copy/deploy the same
 * revision in Apps Script. reference/apps-script/ contains historical
 * snapshots only and is NOT authoritative.
 *
 * Canonicalized: 21.09.2026 from the latest user-supplied live Code.gs.
 */
...
function generateAccessCode() {
  // Math.random() ist fuer Zugangscodes unnoetig schwach/vorhersagbar.
  // Stattdessen SHA-256 ueber zwei UUIDs + Zeitstempel und daraus 12 Zeichen
  // aus einem verwechslungsarmen Alphabet. Bestehende Codes bleiben gueltig.
...
function epubDownloadUrlFor_(file) {
  // EPUB-Dateien bleiben PRIVAT in Drive. Der Browser bekommt nur diesen
  // internen Bezeichner; Lesen und Download laufen ausschliesslich ueber
  // action=getEpubData, wo Zugangscode/Admin-Token serverseitig geprueft
  // werden. Damit umgeht ein geleakter BooksData-Eintrag die Rechte nicht.
  return 'drive-file-id:' + file.getId();
}
...
function syncDriveForAllBooks() {
  ...
  books.forEach(function(b) {
    ...
    if (Object.keys(langs).length) {
      ...
      // Sprach-Umschalter auf der Website automatisch. Die alten      // Top-Level-Felder (b.wordCount/b.hook) bleiben zusätzlich gepflegt,
...
function handle(e) {
  ...
  if (action === 'submit') {
    const person = String(e.parameter.person || '').trim().slice(0, 120);
    const itemType = String(e.parameter.itemType || '').trim().slice(0, 40);
    const itemId = String(e.parameter.itemId || '').trim().slice(0, 240);
    const comment = String(e.parameter.comment || '').trim().slice(0, 4000);
    const starsRaw = String(e.parameter.stars || '').trim();
    const starsNum = starsRaw === '' ? '' : Number(starsRaw);
    if (!person || !itemType || !itemId) return jsonOut({ ok: false, error: 'Unvollstaendige Bewertung.' });
    if (starsRaw !== '' && (!Number.isFinite(starsNum) || starsNum < 1 || starsNum > 5)) return jsonOut({ ok: false, error: 'Ungueltige Sternebewertung.' });
    if (starsRaw === '' && !comment) return jsonOut({ ok: false, error: 'Leere Bewertung.' });
    sheet.appendRow([new Date(), person, itemType, itemId, starsNum, comment]);
